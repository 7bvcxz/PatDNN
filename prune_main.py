...
parser.add_argument('--re_epoch',   default=40, type=int,       help='set retrain epochs')
...
parser.add_argument('--l2',         default=True, action='store_true',  help='apply l3 regularization')
...
#model.load_state_dict(torch.load('./pruned/1e-4_before.pth.tar'), strict=True)
#model.cuda()
...
#with open('./pruned/1e-4_mask.pickle', 'wb') as fw:
#    pickle.dump(mask, fw)

model.load_state_dict(torch.load('./pruned/1e-4_after.pth.tar'), strict=True)
model.cuda()

with open('./pruned/1e-4_mask.pickle', 'rb') as fr:
    mask = pickle.load(fr)
    print("mask loaded")
...
for epoch in range(args.re_epoch):
    ...
    if prec > best_prec:
        ...
        torch.save(model.state_dict(), os.path.join('./pruned/', '1e-4_pruned.pth.tar'))
